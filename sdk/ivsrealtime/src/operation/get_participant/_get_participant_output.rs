// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[cfg_attr(feature = "serde-serialize", derive(::serde::Serialize))]
#[cfg_attr(feature = "serde-deserialize", derive(::serde::Deserialize))]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct GetParticipantOutput {
    /// <p>The participant that is returned.</p>
    pub participant: ::std::option::Option<crate::types::Participant>,
    _request_id: Option<String>,
}
impl GetParticipantOutput {
    /// <p>The participant that is returned.</p>
    pub fn participant(&self) -> ::std::option::Option<&crate::types::Participant> {
        self.participant.as_ref()
    }
}
impl ::aws_types::request_id::RequestId for GetParticipantOutput {
    fn request_id(&self) -> Option<&str> {
        self._request_id.as_deref()
    }
}
impl GetParticipantOutput {
    /// Creates a new builder-style object to manufacture [`GetParticipantOutput`](crate::operation::get_participant::GetParticipantOutput).
    pub fn builder() -> crate::operation::get_participant::builders::GetParticipantOutputBuilder {
        crate::operation::get_participant::builders::GetParticipantOutputBuilder::default()
    }
}

/// A builder for [`GetParticipantOutput`](crate::operation::get_participant::GetParticipantOutput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct GetParticipantOutputBuilder {
    pub(crate) participant: ::std::option::Option<crate::types::Participant>,
    _request_id: Option<String>,
}
impl GetParticipantOutputBuilder {
    /// <p>The participant that is returned.</p>
    pub fn participant(mut self, input: crate::types::Participant) -> Self {
        self.participant = ::std::option::Option::Some(input);
        self
    }
    /// <p>The participant that is returned.</p>
    pub fn set_participant(mut self, input: ::std::option::Option<crate::types::Participant>) -> Self {
        self.participant = input;
        self
    }
    /// <p>The participant that is returned.</p>
    pub fn get_participant(&self) -> &::std::option::Option<crate::types::Participant> {
        &self.participant
    }
    pub(crate) fn _request_id(mut self, request_id: impl Into<String>) -> Self {
        self._request_id = Some(request_id.into());
        self
    }

    pub(crate) fn _set_request_id(&mut self, request_id: Option<String>) -> &mut Self {
        self._request_id = request_id;
        self
    }
    /// Consumes the builder and constructs a [`GetParticipantOutput`](crate::operation::get_participant::GetParticipantOutput).
    pub fn build(self) -> crate::operation::get_participant::GetParticipantOutput {
        crate::operation::get_participant::GetParticipantOutput {
            participant: self.participant,
            _request_id: self._request_id,
        }
    }
}
