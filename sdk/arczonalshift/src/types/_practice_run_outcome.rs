...
/// When writing a match expression against `PracticeRunOutcome`, it is important to ensure
...
///     PracticeRunOutcome::Succeeded => { /* ... */ },
///     other @ _ if other.as_str() == "NewFeature" => { /* handles a case for `NewFeature` */ },
///     _ => { /* ... */ },
/// }
/// ```
/// The above code demonstrates that when `practicerunoutcome` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `PracticeRunOutcome::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `PracticeRunOutcome::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `PracticeRunOutcome::NewFeature` is defined.
/// Specifically, when `practicerunoutcome` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `PracticeRunOutcome::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for PracticeRunOutcome {
    fn from(s: &str) -> Self {
        match s {
            ...
            "SUCCEEDED" => PracticeRunOutcome::Succeeded,
            other => PracticeRunOutcome::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
impl ::std::str::FromStr for PracticeRunOutcome {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(PracticeRunOutcome::from(s))
    }
}
impl PracticeRunOutcome {
    ...
    pub fn as_str(&self) -> &str {
        match self {
            ...
            PracticeRunOutcome::Succeeded => "SUCCEEDED",
            PracticeRunOutcome::Unknown(value) => value.as_str(),
        }
    }
