// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>Content association data for a <a href="https://docs.aws.amazon.com/connect/latest/adminguide/step-by-step-guided-experiences.html">step-by-step guide</a>.</p>
#[non_exhaustive]
#[cfg_attr(feature = "serde-serialize", derive(::serde::Serialize))]
#[cfg_attr(feature = "serde-deserialize", derive(::serde::Deserialize))]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct AmazonConnectGuideAssociationData {
    /// <p>The Amazon Resource Name (ARN) of an Amazon Connect flow. Step-by-step guides are a type of flow.</p>
    pub flow_id: ::std::option::Option<::std::string::String>,
}
impl AmazonConnectGuideAssociationData {
    /// <p>The Amazon Resource Name (ARN) of an Amazon Connect flow. Step-by-step guides are a type of flow.</p>
    pub fn flow_id(&self) -> ::std::option::Option<&str> {
        self.flow_id.as_deref()
    }
}
impl AmazonConnectGuideAssociationData {
    /// Creates a new builder-style object to manufacture [`AmazonConnectGuideAssociationData`](crate::types::AmazonConnectGuideAssociationData).
    pub fn builder() -> crate::types::builders::AmazonConnectGuideAssociationDataBuilder {
        crate::types::builders::AmazonConnectGuideAssociationDataBuilder::default()
    }
}

/// A builder for [`AmazonConnectGuideAssociationData`](crate::types::AmazonConnectGuideAssociationData).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct AmazonConnectGuideAssociationDataBuilder {
    pub(crate) flow_id: ::std::option::Option<::std::string::String>,
}
impl AmazonConnectGuideAssociationDataBuilder {
    /// <p>The Amazon Resource Name (ARN) of an Amazon Connect flow. Step-by-step guides are a type of flow.</p>
    pub fn flow_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.flow_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The Amazon Resource Name (ARN) of an Amazon Connect flow. Step-by-step guides are a type of flow.</p>
    pub fn set_flow_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.flow_id = input;
        self
    }
    /// <p>The Amazon Resource Name (ARN) of an Amazon Connect flow. Step-by-step guides are a type of flow.</p>
    pub fn get_flow_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.flow_id
    }
    /// Consumes the builder and constructs a [`AmazonConnectGuideAssociationData`](crate::types::AmazonConnectGuideAssociationData).
    pub fn build(self) -> crate::types::AmazonConnectGuideAssociationData {
        crate::types::AmazonConnectGuideAssociationData { flow_id: self.flow_id }
    }
}
