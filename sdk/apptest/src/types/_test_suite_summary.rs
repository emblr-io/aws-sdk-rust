// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>Specifies the test suite summary.</p>
#[non_exhaustive]
#[cfg_attr(feature = "serde-serialize", derive(::serde::Serialize))]
#[cfg_attr(feature = "serde-deserialize", derive(::serde::Deserialize))]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct TestSuiteSummary {
    /// <p>The test suite ID of the test suite summary.</p>
    pub test_suite_id: ::std::string::String,
    /// <p>The name of the test suite summary.</p>
    pub name: ::std::string::String,
    /// <p>The status reason of the test suite summary.</p>
    pub status_reason: ::std::option::Option<::std::string::String>,
    /// <p>The latest version of the test suite summary.</p>
    pub latest_version: i32,
    /// <p>The test suite Amazon Resource Name (ARN) of the test suite summary.</p>
    pub test_suite_arn: ::std::string::String,
    /// <p>The status of the test suite summary.</p>
    pub status: crate::types::TestSuiteLifecycle,
    /// <p>The creation time of the test suite summary.</p>
    pub creation_time: ::aws_smithy_types::DateTime,
    /// <p>The last update time of the test suite summary.</p>
    pub last_update_time: ::aws_smithy_types::DateTime,
}
impl TestSuiteSummary {
    /// <p>The test suite ID of the test suite summary.</p>
    pub fn test_suite_id(&self) -> &str {
        use std::ops::Deref;
        self.test_suite_id.deref()
    }
    /// <p>The name of the test suite summary.</p>
    pub fn name(&self) -> &str {
        use std::ops::Deref;
        self.name.deref()
    }
    /// <p>The status reason of the test suite summary.</p>
    pub fn status_reason(&self) -> ::std::option::Option<&str> {
        self.status_reason.as_deref()
    }
    /// <p>The latest version of the test suite summary.</p>
    pub fn latest_version(&self) -> i32 {
        self.latest_version
    }
    /// <p>The test suite Amazon Resource Name (ARN) of the test suite summary.</p>
    pub fn test_suite_arn(&self) -> &str {
        use std::ops::Deref;
        self.test_suite_arn.deref()
    }
    /// <p>The status of the test suite summary.</p>
    pub fn status(&self) -> &crate::types::TestSuiteLifecycle {
        &self.status
    }
    /// <p>The creation time of the test suite summary.</p>
    pub fn creation_time(&self) -> &::aws_smithy_types::DateTime {
        &self.creation_time
    }
    /// <p>The last update time of the test suite summary.</p>
    pub fn last_update_time(&self) -> &::aws_smithy_types::DateTime {
        &self.last_update_time
    }
}
impl TestSuiteSummary {
    /// Creates a new builder-style object to manufacture [`TestSuiteSummary`](crate::types::TestSuiteSummary).
    pub fn builder() -> crate::types::builders::TestSuiteSummaryBuilder {
        crate::types::builders::TestSuiteSummaryBuilder::default()
    }
}

/// A builder for [`TestSuiteSummary`](crate::types::TestSuiteSummary).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct TestSuiteSummaryBuilder {
    pub(crate) test_suite_id: ::std::option::Option<::std::string::String>,
    pub(crate) name: ::std::option::Option<::std::string::String>,
    pub(crate) status_reason: ::std::option::Option<::std::string::String>,
    pub(crate) latest_version: ::std::option::Option<i32>,
    pub(crate) test_suite_arn: ::std::option::Option<::std::string::String>,
    pub(crate) status: ::std::option::Option<crate::types::TestSuiteLifecycle>,
    pub(crate) creation_time: ::std::option::Option<::aws_smithy_types::DateTime>,
    pub(crate) last_update_time: ::std::option::Option<::aws_smithy_types::DateTime>,
}
impl TestSuiteSummaryBuilder {
    /// <p>The test suite ID of the test suite summary.</p>
    /// This field is required.
    pub fn test_suite_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.test_suite_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The test suite ID of the test suite summary.</p>
    pub fn set_test_suite_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.test_suite_id = input;
        self
    }
    /// <p>The test suite ID of the test suite summary.</p>
    pub fn get_test_suite_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.test_suite_id
    }
    /// <p>The name of the test suite summary.</p>
    /// This field is required.
    pub fn name(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.name = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The name of the test suite summary.</p>
    pub fn set_name(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.name = input;
        self
    }
    /// <p>The name of the test suite summary.</p>
    pub fn get_name(&self) -> &::std::option::Option<::std::string::String> {
        &self.name
    }
    /// <p>The status reason of the test suite summary.</p>
    pub fn status_reason(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.status_reason = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The status reason of the test suite summary.</p>
    pub fn set_status_reason(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.status_reason = input;
        self
    }
    /// <p>The status reason of the test suite summary.</p>
    pub fn get_status_reason(&self) -> &::std::option::Option<::std::string::String> {
        &self.status_reason
    }
    /// <p>The latest version of the test suite summary.</p>
    /// This field is required.
    pub fn latest_version(mut self, input: i32) -> Self {
        self.latest_version = ::std::option::Option::Some(input);
        self
    }
    /// <p>The latest version of the test suite summary.</p>
    pub fn set_latest_version(mut self, input: ::std::option::Option<i32>) -> Self {
        self.latest_version = input;
        self
    }
    /// <p>The latest version of the test suite summary.</p>
    pub fn get_latest_version(&self) -> &::std::option::Option<i32> {
        &self.latest_version
    }
    /// <p>The test suite Amazon Resource Name (ARN) of the test suite summary.</p>
    /// This field is required.
    pub fn test_suite_arn(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.test_suite_arn = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The test suite Amazon Resource Name (ARN) of the test suite summary.</p>
    pub fn set_test_suite_arn(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.test_suite_arn = input;
        self
    }
    /// <p>The test suite Amazon Resource Name (ARN) of the test suite summary.</p>
    pub fn get_test_suite_arn(&self) -> &::std::option::Option<::std::string::String> {
        &self.test_suite_arn
    }
    /// <p>The status of the test suite summary.</p>
    /// This field is required.
    pub fn status(mut self, input: crate::types::TestSuiteLifecycle) -> Self {
        self.status = ::std::option::Option::Some(input);
        self
    }
    /// <p>The status of the test suite summary.</p>
    pub fn set_status(mut self, input: ::std::option::Option<crate::types::TestSuiteLifecycle>) -> Self {
        self.status = input;
        self
    }
    /// <p>The status of the test suite summary.</p>
    pub fn get_status(&self) -> &::std::option::Option<crate::types::TestSuiteLifecycle> {
        &self.status
    }
    /// <p>The creation time of the test suite summary.</p>
    /// This field is required.
    pub fn creation_time(mut self, input: ::aws_smithy_types::DateTime) -> Self {
        self.creation_time = ::std::option::Option::Some(input);
        self
    }
    /// <p>The creation time of the test suite summary.</p>
    pub fn set_creation_time(mut self, input: ::std::option::Option<::aws_smithy_types::DateTime>) -> Self {
        self.creation_time = input;
        self
    }
    /// <p>The creation time of the test suite summary.</p>
    pub fn get_creation_time(&self) -> &::std::option::Option<::aws_smithy_types::DateTime> {
        &self.creation_time
    }
    /// <p>The last update time of the test suite summary.</p>
    /// This field is required.
    pub fn last_update_time(mut self, input: ::aws_smithy_types::DateTime) -> Self {
        self.last_update_time = ::std::option::Option::Some(input);
        self
    }
    /// <p>The last update time of the test suite summary.</p>
    pub fn set_last_update_time(mut self, input: ::std::option::Option<::aws_smithy_types::DateTime>) -> Self {
        self.last_update_time = input;
        self
    }
    /// <p>The last update time of the test suite summary.</p>
    pub fn get_last_update_time(&self) -> &::std::option::Option<::aws_smithy_types::DateTime> {
        &self.last_update_time
    }
    /// Consumes the builder and constructs a [`TestSuiteSummary`](crate::types::TestSuiteSummary).
    /// This method will fail if any of the following fields are not set:
    /// - [`test_suite_id`](crate::types::builders::TestSuiteSummaryBuilder::test_suite_id)
    /// - [`name`](crate::types::builders::TestSuiteSummaryBuilder::name)
    /// - [`latest_version`](crate::types::builders::TestSuiteSummaryBuilder::latest_version)
    /// - [`test_suite_arn`](crate::types::builders::TestSuiteSummaryBuilder::test_suite_arn)
    /// - [`status`](crate::types::builders::TestSuiteSummaryBuilder::status)
    /// - [`creation_time`](crate::types::builders::TestSuiteSummaryBuilder::creation_time)
    /// - [`last_update_time`](crate::types::builders::TestSuiteSummaryBuilder::last_update_time)
    pub fn build(self) -> ::std::result::Result<crate::types::TestSuiteSummary, ::aws_smithy_types::error::operation::BuildError> {
        ::std::result::Result::Ok(crate::types::TestSuiteSummary {
            test_suite_id: self.test_suite_id.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "test_suite_id",
                    "test_suite_id was not specified but it is required when building TestSuiteSummary",
                )
            })?,
            name: self.name.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "name",
                    "name was not specified but it is required when building TestSuiteSummary",
                )
            })?,
            status_reason: self.status_reason,
            latest_version: self.latest_version.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "latest_version",
                    "latest_version was not specified but it is required when building TestSuiteSummary",
                )
            })?,
            test_suite_arn: self.test_suite_arn.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "test_suite_arn",
                    "test_suite_arn was not specified but it is required when building TestSuiteSummary",
                )
            })?,
            status: self.status.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "status",
                    "status was not specified but it is required when building TestSuiteSummary",
                )
            })?,
            creation_time: self.creation_time.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "creation_time",
                    "creation_time was not specified but it is required when building TestSuiteSummary",
                )
            })?,
            last_update_time: self.last_update_time.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "last_update_time",
                    "last_update_time was not specified but it is required when building TestSuiteSummary",
                )
            })?,
        })
    }
}
