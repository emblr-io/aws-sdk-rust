...
//! offered by AWS Mainframe Modernization Application Testing. The return value of each of these methods is a "fluent builder",
...
/// Client for calling AWS Mainframe Modernization Application Testing.
...
/// For example, the [`CreateTestCase`](crate::operation::create_test_case) operation has
/// a [`Client::create_test_case`], function which returns a builder for that operation.
/// The fluent builder ultimately has a `send()` function that returns an async future that
/// returns a result, as illustrated below:
///
/// ```rust,ignore
/// let result = client.create_test_case()
///     .name("example")
///     .send()
///     .await;
/// ```
...
/// Configuration for AWS Mainframe Modernization Application Testing.
pub mod config;
