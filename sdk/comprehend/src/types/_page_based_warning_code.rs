...
/// When writing a match expression against `PageBasedWarningCode`, it is important to ensure
...
///
/// ```text
/// # let pagebasedwarningcode = unimplemented!();
/// match pagebasedwarningcode {
...
/// }
/// ```
/// The above code demonstrates that when `pagebasedwarningcode` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `PageBasedWarningCode::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `PageBasedWarningCode::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `PageBasedWarningCode::NewFeature` is defined.
/// Specifically, when `pagebasedwarningcode` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `PageBasedWarningCode::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for PageBasedWarningCode {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => PageBasedWarningCode::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
impl ::std::str::FromStr for PageBasedWarningCode {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(PageBasedWarningCode::from(s))
    }
}
impl PageBasedWarningCode {
...
impl ::std::convert::AsRef<str> for PageBasedWarningCode {
    fn as_ref(&self) -> &str {
        self.as_str()
    }
}
