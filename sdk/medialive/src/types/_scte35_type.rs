...
/// When writing a match expression against `Scte35Type`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `scte35type` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `Scte35Type::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `Scte35Type::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `Scte35Type::NewFeature` is defined.
/// Specifically, when `scte35type` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `Scte35Type::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for Scte35Type {
    fn from(s: &str) -> Self {
        match s {
...
impl ::std::str::FromStr for Scte35Type {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(Scte35Type::from(s))
    }
}
impl Scte35Type {
...
impl ::std::fmt::Display for Scte35Type {
