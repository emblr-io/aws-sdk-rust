...
/// When writing a match expression against `WorkloadEstimateRateType`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `workloadestimateratetype` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `WorkloadEstimateRateType::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `WorkloadEstimateRateType::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `WorkloadEstimateRateType::NewFeature` is defined.
/// Specifically, when `workloadestimateratetype` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `WorkloadEstimateRateType::NewFeature` also yielding `"NewFeature"`.
///
...
pub enum WorkloadEstimateRateType {
    #[allow(missing_docs)] // documentation missing in model
    AfterDiscounts,
    #[allow(missing_docs)] // documentation missing in model
...
impl ::std::convert::From<&str> for WorkloadEstimateRateType {
    fn from(s: &str) -> Self {
        match s {
...
impl ::std::str::FromStr for WorkloadEstimateRateType {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(WorkloadEstimateRateType::from(s))
    }
}
impl WorkloadEstimateRateType {
...
impl ::std::fmt::Display for WorkloadEstimateRateType {
