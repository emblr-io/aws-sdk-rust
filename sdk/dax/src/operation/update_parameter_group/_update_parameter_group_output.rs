// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[cfg_attr(feature = "serde-serialize", derive(::serde::Serialize))]
#[cfg_attr(feature = "serde-deserialize", derive(::serde::Deserialize))]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct UpdateParameterGroupOutput {
    /// <p>The parameter group that has been modified.</p>
    pub parameter_group: ::std::option::Option<crate::types::ParameterGroup>,
    _request_id: Option<String>,
}
impl UpdateParameterGroupOutput {
    /// <p>The parameter group that has been modified.</p>
    pub fn parameter_group(&self) -> ::std::option::Option<&crate::types::ParameterGroup> {
        self.parameter_group.as_ref()
    }
}
impl ::aws_types::request_id::RequestId for UpdateParameterGroupOutput {
    fn request_id(&self) -> Option<&str> {
        self._request_id.as_deref()
    }
}
impl UpdateParameterGroupOutput {
    /// Creates a new builder-style object to manufacture [`UpdateParameterGroupOutput`](crate::operation::update_parameter_group::UpdateParameterGroupOutput).
    pub fn builder() -> crate::operation::update_parameter_group::builders::UpdateParameterGroupOutputBuilder {
        crate::operation::update_parameter_group::builders::UpdateParameterGroupOutputBuilder::default()
    }
}

/// A builder for [`UpdateParameterGroupOutput`](crate::operation::update_parameter_group::UpdateParameterGroupOutput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct UpdateParameterGroupOutputBuilder {
    pub(crate) parameter_group: ::std::option::Option<crate::types::ParameterGroup>,
    _request_id: Option<String>,
}
impl UpdateParameterGroupOutputBuilder {
    /// <p>The parameter group that has been modified.</p>
    pub fn parameter_group(mut self, input: crate::types::ParameterGroup) -> Self {
        self.parameter_group = ::std::option::Option::Some(input);
        self
    }
    /// <p>The parameter group that has been modified.</p>
    pub fn set_parameter_group(mut self, input: ::std::option::Option<crate::types::ParameterGroup>) -> Self {
        self.parameter_group = input;
        self
    }
    /// <p>The parameter group that has been modified.</p>
    pub fn get_parameter_group(&self) -> &::std::option::Option<crate::types::ParameterGroup> {
        &self.parameter_group
    }
    pub(crate) fn _request_id(mut self, request_id: impl Into<String>) -> Self {
        self._request_id = Some(request_id.into());
        self
    }

    pub(crate) fn _set_request_id(&mut self, request_id: Option<String>) -> &mut Self {
        self._request_id = request_id;
        self
    }
    /// Consumes the builder and constructs a [`UpdateParameterGroupOutput`](crate::operation::update_parameter_group::UpdateParameterGroupOutput).
    pub fn build(self) -> crate::operation::update_parameter_group::UpdateParameterGroupOutput {
        crate::operation::update_parameter_group::UpdateParameterGroupOutput {
            parameter_group: self.parameter_group,
            _request_id: self._request_id,
        }
    }
}
