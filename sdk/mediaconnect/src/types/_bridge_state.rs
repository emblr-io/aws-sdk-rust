...
/// When writing a match expression against `BridgeState`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `bridgestate` represents
...
/// version of SDK where the variant `BridgeState::NewFeature` is defined.
...
impl ::std::convert::From<&str> for BridgeState {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => BridgeState::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
