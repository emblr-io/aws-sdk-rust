...
/// When writing a match expression against `SaaSProductSortBy`, it is important to ensure
...
///     SaaSProductSortBy::Visibility => { /* ... */ },
///     other @ _ if other.as_str() == "NewFeature" => { /* handles a case for `NewFeature` */ },
///     _ => { /* ... */ },
/// }
/// ```
/// The above code demonstrates that when `saasproductsortby` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `SaaSProductSortBy::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `SaaSProductSortBy::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `SaaSProductSortBy::NewFeature` is defined.
/// Specifically, when `saasproductsortby` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `SaaSProductSortBy::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for SaaSProductSortBy {
    fn from(s: &str) -> Self {
        match s {
            ...
            "Visibility" => SaaSProductSortBy::Visibility,
            other => SaaSProductSortBy::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
impl ::std::str::FromStr for SaaSProductSortBy {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(SaaSProductSortBy::from(s))
    }
}
impl SaaSProductSortBy {
    ...
    pub fn as_str(&self) -> &str {
        match self {
            ...
            SaaSProductSortBy::Visibility => "Visibility",
            SaaSProductSortBy::Unknown(value) => value.as_str(),
        }
    }
