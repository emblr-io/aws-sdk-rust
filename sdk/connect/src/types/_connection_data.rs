// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>Information required to join the call.</p>
#[non_exhaustive]
#[cfg_attr(feature = "serde-serialize", derive(::serde::Serialize))]
#[cfg_attr(feature = "serde-deserialize", derive(::serde::Deserialize))]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct ConnectionData {
    /// <p>The attendee information, including attendee ID and join token.</p>
    pub attendee: ::std::option::Option<crate::types::Attendee>,
    /// <p>A meeting created using the Amazon Chime SDK.</p>
    pub meeting: ::std::option::Option<crate::types::Meeting>,
}
impl ConnectionData {
    /// <p>The attendee information, including attendee ID and join token.</p>
    pub fn attendee(&self) -> ::std::option::Option<&crate::types::Attendee> {
        self.attendee.as_ref()
    }
    /// <p>A meeting created using the Amazon Chime SDK.</p>
    pub fn meeting(&self) -> ::std::option::Option<&crate::types::Meeting> {
        self.meeting.as_ref()
    }
}
impl ConnectionData {
    /// Creates a new builder-style object to manufacture [`ConnectionData`](crate::types::ConnectionData).
    pub fn builder() -> crate::types::builders::ConnectionDataBuilder {
        crate::types::builders::ConnectionDataBuilder::default()
    }
}

/// A builder for [`ConnectionData`](crate::types::ConnectionData).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct ConnectionDataBuilder {
    pub(crate) attendee: ::std::option::Option<crate::types::Attendee>,
    pub(crate) meeting: ::std::option::Option<crate::types::Meeting>,
}
impl ConnectionDataBuilder {
    /// <p>The attendee information, including attendee ID and join token.</p>
    pub fn attendee(mut self, input: crate::types::Attendee) -> Self {
        self.attendee = ::std::option::Option::Some(input);
        self
    }
    /// <p>The attendee information, including attendee ID and join token.</p>
    pub fn set_attendee(mut self, input: ::std::option::Option<crate::types::Attendee>) -> Self {
        self.attendee = input;
        self
    }
    /// <p>The attendee information, including attendee ID and join token.</p>
    pub fn get_attendee(&self) -> &::std::option::Option<crate::types::Attendee> {
        &self.attendee
    }
    /// <p>A meeting created using the Amazon Chime SDK.</p>
    pub fn meeting(mut self, input: crate::types::Meeting) -> Self {
        self.meeting = ::std::option::Option::Some(input);
        self
    }
    /// <p>A meeting created using the Amazon Chime SDK.</p>
    pub fn set_meeting(mut self, input: ::std::option::Option<crate::types::Meeting>) -> Self {
        self.meeting = input;
        self
    }
    /// <p>A meeting created using the Amazon Chime SDK.</p>
    pub fn get_meeting(&self) -> &::std::option::Option<crate::types::Meeting> {
        &self.meeting
    }
    /// Consumes the builder and constructs a [`ConnectionData`](crate::types::ConnectionData).
    pub fn build(self) -> crate::types::ConnectionData {
        crate::types::ConnectionData {
            attendee: self.attendee,
            meeting: self.meeting,
        }
    }
}
