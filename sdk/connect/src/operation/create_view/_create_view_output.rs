// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[cfg_attr(feature = "serde-serialize", derive(::serde::Serialize))]
#[cfg_attr(feature = "serde-deserialize", derive(::serde::Deserialize))]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct CreateViewOutput {
    /// <p>A view resource object. Contains metadata and content necessary to render the view.</p>
    pub view: ::std::option::Option<crate::types::View>,
    _request_id: Option<String>,
}
impl CreateViewOutput {
    /// <p>A view resource object. Contains metadata and content necessary to render the view.</p>
    pub fn view(&self) -> ::std::option::Option<&crate::types::View> {
        self.view.as_ref()
    }
}
impl ::aws_types::request_id::RequestId for CreateViewOutput {
    fn request_id(&self) -> Option<&str> {
        self._request_id.as_deref()
    }
}
impl CreateViewOutput {
    /// Creates a new builder-style object to manufacture [`CreateViewOutput`](crate::operation::create_view::CreateViewOutput).
    pub fn builder() -> crate::operation::create_view::builders::CreateViewOutputBuilder {
        crate::operation::create_view::builders::CreateViewOutputBuilder::default()
    }
}

/// A builder for [`CreateViewOutput`](crate::operation::create_view::CreateViewOutput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct CreateViewOutputBuilder {
    pub(crate) view: ::std::option::Option<crate::types::View>,
    _request_id: Option<String>,
}
impl CreateViewOutputBuilder {
    /// <p>A view resource object. Contains metadata and content necessary to render the view.</p>
    pub fn view(mut self, input: crate::types::View) -> Self {
        self.view = ::std::option::Option::Some(input);
        self
    }
    /// <p>A view resource object. Contains metadata and content necessary to render the view.</p>
    pub fn set_view(mut self, input: ::std::option::Option<crate::types::View>) -> Self {
        self.view = input;
        self
    }
    /// <p>A view resource object. Contains metadata and content necessary to render the view.</p>
    pub fn get_view(&self) -> &::std::option::Option<crate::types::View> {
        &self.view
    }
    pub(crate) fn _request_id(mut self, request_id: impl Into<String>) -> Self {
        self._request_id = Some(request_id.into());
        self
    }

    pub(crate) fn _set_request_id(&mut self, request_id: Option<String>) -> &mut Self {
        self._request_id = request_id;
        self
    }
    /// Consumes the builder and constructs a [`CreateViewOutput`](crate::operation::create_view::CreateViewOutput).
    pub fn build(self) -> crate::operation::create_view::CreateViewOutput {
        crate::operation::create_view::CreateViewOutput {
            view: self.view,
            _request_id: self._request_id,
        }
    }
}
