...
/// When writing a match expression against `ConnectorState`, it is important to ensure
...
///
/// ```text
/// # let connectorstate = unimplemented!();
/// match connectorstate {
...
/// }
/// ```
/// The above code demonstrates that when `connectorstate` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `ConnectorState::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `ConnectorState::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `ConnectorState::NewFeature` is defined.
/// Specifically, when `connectorstate` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `ConnectorState::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::str::FromStr for ConnectorState {
    ...
    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(ConnectorState::from(s))
    }
}
impl ConnectorState {
