...
pub struct DeliveryStreamEncryptionConfigurationInput {
    ...
    /// <p>When you invoke <code>StartDeliveryStreamEncryption</code> to change the CMK for a Firehose stream that is encrypted with a customer managed CMK, Firehose schedules the grant it had on the old CMK for retirement.</p>
    /// <p>You can use a CMK of type CUSTOMER_MANAGED_CMK to encrypt up to 500 Firehose streams. If a <code>CreateDeliveryStream</code> or <code>StartDeliveryStreamEncryption</code> operation exceeds this limit, Firehose throws a <code>LimitExceededException</code>.</p><important>
