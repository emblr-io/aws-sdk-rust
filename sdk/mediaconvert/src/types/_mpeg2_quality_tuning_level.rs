...
/// When writing a match expression against `Mpeg2QualityTuningLevel`, it is important to ensure
...
///     Mpeg2QualityTuningLevel::MultiPass => { /* ... */ },
///     Mpeg2QualityTuningLevel::SinglePass => { /* ... */ },
///     other @ _ if other.as_str() == "NewFeature" => { /* handles a case for `NewFeature` */ },
///     _ => { /* ... */ },
/// }
/// ```
/// The above code demonstrates that when `mpeg2qualitytuninglevel` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `Mpeg2QualityTuningLevel::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `Mpeg2QualityTuningLevel::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `Mpeg2QualityTuningLevel::NewFeature` is defined.
...
impl ::std::convert::From<&str> for Mpeg2QualityTuningLevel {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => Mpeg2QualityTuningLevel::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
