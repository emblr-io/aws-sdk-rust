...
/// When writing a match expression against `XavcSpatialAdaptiveQuantization`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `xavcspatialadaptivequantization` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `XavcSpatialAdaptiveQuantization::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `XavcSpatialAdaptiveQuantization::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `XavcSpatialAdaptiveQuantization::NewFeature` is defined.
/// Specifically, when `xavcspatialadaptivequantization` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `XavcSpatialAdaptiveQuantization::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for XavcSpatialAdaptiveQuantization {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => XavcSpatialAdaptiveQuantization::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
...
impl XavcSpatialAdaptiveQuantization {
    ...
    pub fn as_str(&self) -> &str {
        match self {
            ...
            XavcSpatialAdaptiveQuantization::Unknown(value) => value.as_str(),
        }
    }
