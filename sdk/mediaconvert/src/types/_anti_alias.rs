...
/// When writing a match expression against `AntiAlias`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `antialias` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `AntiAlias::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `AntiAlias::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `AntiAlias::NewFeature` is defined.
...
impl ::std::convert::From<&str> for AntiAlias {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => AntiAlias::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
