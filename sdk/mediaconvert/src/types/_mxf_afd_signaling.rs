...
/// When writing a match expression against `MxfAfdSignaling`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `mxfafdsignaling` represents
...
/// version of SDK where the variant `MxfAfdSignaling::NewFeature` is defined.
...
impl ::std::convert::From<&str> for MxfAfdSignaling {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => MxfAfdSignaling::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
