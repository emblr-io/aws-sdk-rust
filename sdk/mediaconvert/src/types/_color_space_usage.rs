...
/// When writing a match expression against `ColorSpaceUsage`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `colorspaceusage` represents
...
/// version of SDK where the variant `ColorSpaceUsage::NewFeature` is defined.
/// Specifically, when `colorspaceusage` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `ColorSpaceUsage::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for ColorSpaceUsage {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => ColorSpaceUsage::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
...
impl ColorSpaceUsage {
    ...
    pub fn as_str(&self) -> &str {
        match self {
            ...
            ColorSpaceUsage::Unknown(value) => value.as_str(),
        }
    }
