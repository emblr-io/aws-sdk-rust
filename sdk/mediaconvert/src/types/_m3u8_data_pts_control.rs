...
/// When writing a match expression against `M3u8DataPtsControl`, it is important to ensure
...
///     M3u8DataPtsControl::AlignToVideo => { /* ... */ },
///     M3u8DataPtsControl::Auto => { /* ... */ },
///     other @ _ if other.as_str() == "NewFeature" => { /* handles a case for `NewFeature` */ },
///     _ => { /* ... */ },
/// }
/// ```
/// The above code demonstrates that when `m3u8dataptscontrol` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `M3u8DataPtsControl::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `M3u8DataPtsControl::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `M3u8DataPtsControl::NewFeature` is defined.
...
impl ::std::convert::From<&str> for M3u8DataPtsControl {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => M3u8DataPtsControl::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
impl ::std::str::FromStr for M3u8DataPtsControl {
    ...
    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(M3u8DataPtsControl::from(s))
    }
}
...
impl ::std::convert::AsRef<str> for M3u8DataPtsControl {
    fn as_ref(&self) -> &str {
        self.as_str()
    }
}
