...
/// When writing a match expression against `HlsInitializationVectorInManifest`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `hlsinitializationvectorinmanifest` represents
...
/// version of SDK where the variant `HlsInitializationVectorInManifest::NewFeature` is defined.
/// Specifically, when `hlsinitializationvectorinmanifest` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `HlsInitializationVectorInManifest::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for HlsInitializationVectorInManifest {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => HlsInitializationVectorInManifest::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
...
impl HlsInitializationVectorInManifest {
    ...
    pub fn as_str(&self) -> &str {
        match self {
            ...
            HlsInitializationVectorInManifest::Unknown(value) => value.as_str(),
        }
    }
