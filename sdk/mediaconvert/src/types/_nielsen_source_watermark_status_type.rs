...
/// When writing a match expression against `NielsenSourceWatermarkStatusType`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `nielsensourcewatermarkstatustype` represents
...
/// version of SDK where the variant `NielsenSourceWatermarkStatusType::NewFeature` is defined.
/// Specifically, when `nielsensourcewatermarkstatustype` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `NielsenSourceWatermarkStatusType::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for NielsenSourceWatermarkStatusType {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => NielsenSourceWatermarkStatusType::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
...
impl NielsenSourceWatermarkStatusType {
    ...
    pub fn as_str(&self) -> &str {
        match self {
            ...
            NielsenSourceWatermarkStatusType::Unknown(value) => value.as_str(),
        }
    }
