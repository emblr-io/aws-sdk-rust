...
/// When writing a match expression against `Commitment`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `commitment` represents
...
/// version of SDK where the variant `Commitment::NewFeature` is defined.
/// Specifically, when `commitment` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `Commitment::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for Commitment {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => Commitment::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
...
impl Commitment {
    ...
    pub fn as_str(&self) -> &str {
        match self {
            ...
            Commitment::Unknown(value) => value.as_str(),
        }
    }
