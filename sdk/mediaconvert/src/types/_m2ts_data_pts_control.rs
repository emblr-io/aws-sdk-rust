...
/// When writing a match expression against `M2tsDataPtsControl`, it is important to ensure
...
///
/// ```text
/// # let m2tsdataptscontrol = unimplemented!();
/// match m2tsdataptscontrol {
///     M2tsDataPtsControl::AlignToVideo => { /* ... */ },
///     M2tsDataPtsControl::Auto => { /* ... */ },
///     other @ _ if other.as_str() == "NewFeature" => { /* handles a case for `NewFeature` */ },
///     _ => { /* ... */ },
/// }
/// ```
/// The above code demonstrates that when `m2tsdataptscontrol` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `M2tsDataPtsControl::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `M2tsDataPtsControl::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `M2tsDataPtsControl::NewFeature` is defined.
/// Specifically, when `m2tsdataptscontrol` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `M2tsDataPtsControl::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for M2tsDataPtsControl {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => M2tsDataPtsControl::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
...
impl M2tsDataPtsControl {
    ...
    pub fn as_str(&self) -> &str {
        match self {
            ...
            M2tsDataPtsControl::Unknown(value) => value.as_str(),
        }
    }
