...
/// When writing a match expression against `TimedMetadata`, it is important to ensure
...
///
/// ```text
/// # let timedmetadata = unimplemented!();
/// match timedmetadata {
///     TimedMetadata::None => { /* ... */ },
...
/// }
/// ```
/// The above code demonstrates that when `timedmetadata` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `TimedMetadata::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `TimedMetadata::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `TimedMetadata::NewFeature` is defined.
/// Specifically, when `timedmetadata` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `TimedMetadata::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for TimedMetadata {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => TimedMetadata::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
...
impl TimedMetadata {
    ...
    pub fn as_str(&self) -> &str {
        match self {
            ...
            TimedMetadata::Unknown(value) => value.as_str(),
        }
    }
