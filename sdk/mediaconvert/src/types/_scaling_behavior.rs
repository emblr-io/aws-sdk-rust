...
/// When writing a match expression against `ScalingBehavior`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `scalingbehavior` represents
...
/// version of SDK where the variant `ScalingBehavior::NewFeature` is defined.
/// Specifically, when `scalingbehavior` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `ScalingBehavior::NewFeature` also yielding `"NewFeature"`.
///
