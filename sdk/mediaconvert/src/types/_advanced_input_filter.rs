...
/// When writing a match expression against `AdvancedInputFilter`, it is important to ensure
...
///     AdvancedInputFilter::Disabled => { /* ... */ },
///     AdvancedInputFilter::Enabled => { /* ... */ },
///     other @ _ if other.as_str() == "NewFeature" => { /* handles a case for `NewFeature` */ },
///     _ => { /* ... */ },
/// }
/// ```
/// The above code demonstrates that when `advancedinputfilter` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `AdvancedInputFilter::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `AdvancedInputFilter::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `AdvancedInputFilter::NewFeature` is defined.
...
impl ::std::convert::From<&str> for AdvancedInputFilter {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => AdvancedInputFilter::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
