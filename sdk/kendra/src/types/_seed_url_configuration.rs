...
pub struct SeedUrlConfiguration {
    ...
    /// <ul>
    /// <li>
    /// <p><code>HOST_ONLY</code>—crawl only the website host names. For example, if the seed URL is "abc.example.com", then only URLs with host name "abc.example.com" are crawled.</p></li>
    /// <li>
    /// <p><code>SUBDOMAINS</code>—crawl the website host names with subdomains. For example, if the seed URL is "abc.example.com", then "a.abc.example.com" and "b.abc.example.com" are also crawled.</p></li>
    /// <li>
...
impl SeedUrlConfigurationBuilder {
    ...
    pub fn web_crawler_mode(mut self, input: crate::types::WebCrawlerMode) -> Self {
        self.web_crawler_mode = ::std::option::Option::Some(input);
        self
    }
    /// <p>You can choose one of the following modes:</p>
    /// <ul>
