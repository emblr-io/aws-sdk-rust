...
/// When writing a match expression against `IpamDiscoveryFailureCode`, it is important to ensure
...
///
/// ```text
/// # let ipamdiscoveryfailurecode = unimplemented!();
/// match ipamdiscoveryfailurecode {
...
/// }
/// ```
/// The above code demonstrates that when `ipamdiscoveryfailurecode` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `IpamDiscoveryFailureCode::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `IpamDiscoveryFailureCode::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `IpamDiscoveryFailureCode::NewFeature` is defined.
/// Specifically, when `ipamdiscoveryfailurecode` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `IpamDiscoveryFailureCode::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for IpamDiscoveryFailureCode {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => IpamDiscoveryFailureCode::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
...
impl IpamDiscoveryFailureCode {
    ...
    pub fn as_str(&self) -> &str {
        match self {
            IpamDiscoveryFailureCode::AssumeRoleFailure => "assume-role-failure",
            IpamDiscoveryFailureCode::ThrottlingFailure => "throttling-failure",
            IpamDiscoveryFailureCode::UnauthorizedFailure => "unauthorized-failure",
            IpamDiscoveryFailureCode::Unknown(value) => value.as_str(),
        }
    }
