...
/// When writing a match expression against `SftpAuthenticationMethods`, it is important to ensure
...
///     SftpAuthenticationMethods::PublicKey => { /* ... */ },
///     SftpAuthenticationMethods::PublicKeyAndPassword => { /* ... */ },
///     SftpAuthenticationMethods::PublicKeyOrPassword => { /* ... */ },
///     other @ _ if other.as_str() == "NewFeature" => { /* handles a case for `NewFeature` */ },
///     _ => { /* ... */ },
/// }
/// ```
/// The above code demonstrates that when `sftpauthenticationmethods` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `SftpAuthenticationMethods::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `SftpAuthenticationMethods::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `SftpAuthenticationMethods::NewFeature` is defined.
