...
/// When writing a match expression against `RetrievalResultContentType`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `retrievalresultcontenttype` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `RetrievalResultContentType::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `RetrievalResultContentType::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `RetrievalResultContentType::NewFeature` is defined.
/// Specifically, when `retrievalresultcontenttype` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `RetrievalResultContentType::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::str::FromStr for RetrievalResultContentType {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(RetrievalResultContentType::from(s))
    }
}
impl RetrievalResultContentType {
