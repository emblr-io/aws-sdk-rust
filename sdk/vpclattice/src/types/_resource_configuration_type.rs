...
/// When writing a match expression against `ResourceConfigurationType`, it is important to ensure
...
///
/// ```text
/// # let resourceconfigurationtype = unimplemented!();
...
/// }
/// ```
/// The above code demonstrates that when `resourceconfigurationtype` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `ResourceConfigurationType::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `ResourceConfigurationType::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `ResourceConfigurationType::NewFeature` is defined.
/// Specifically, when `resourceconfigurationtype` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `ResourceConfigurationType::NewFeature` also yielding `"NewFeature"`.
///
...
pub enum ResourceConfigurationType {
...
impl ::std::convert::From<&str> for ResourceConfigurationType {
    fn from(s: &str) -> Self {
        match s {
...
impl ::std::str::FromStr for ResourceConfigurationType {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(ResourceConfigurationType::from(s))
    }
}
impl ResourceConfigurationType {
...
impl ::std::fmt::Display for ResourceConfigurationType {
