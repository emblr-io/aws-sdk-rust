...
/// When writing a match expression against `TargetGroupProtocol`, it is important to ensure
...
///
/// ```text
/// # let targetgroupprotocol = unimplemented!();
...
/// }
/// ```
/// The above code demonstrates that when `targetgroupprotocol` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `TargetGroupProtocol::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `TargetGroupProtocol::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `TargetGroupProtocol::NewFeature` is defined.
/// Specifically, when `targetgroupprotocol` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `TargetGroupProtocol::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::fmt::Display for TargetGroupProtocol {
