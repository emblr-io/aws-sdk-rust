// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[cfg_attr(feature = "serde-serialize", derive(::serde::Serialize))]
#[cfg_attr(feature = "serde-deserialize", derive(::serde::Deserialize))]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct UpdateHubContentReferenceOutput {
    /// <p>The ARN of the private model hub that contains the updated hub content.</p>
    pub hub_arn: ::std::option::Option<::std::string::String>,
    /// <p>The ARN of the hub content resource that was updated.</p>
    pub hub_content_arn: ::std::option::Option<::std::string::String>,
    _request_id: Option<String>,
}
impl UpdateHubContentReferenceOutput {
    /// <p>The ARN of the private model hub that contains the updated hub content.</p>
    pub fn hub_arn(&self) -> ::std::option::Option<&str> {
        self.hub_arn.as_deref()
    }
    /// <p>The ARN of the hub content resource that was updated.</p>
    pub fn hub_content_arn(&self) -> ::std::option::Option<&str> {
        self.hub_content_arn.as_deref()
    }
}
impl ::aws_types::request_id::RequestId for UpdateHubContentReferenceOutput {
    fn request_id(&self) -> Option<&str> {
        self._request_id.as_deref()
    }
}
impl UpdateHubContentReferenceOutput {
    /// Creates a new builder-style object to manufacture [`UpdateHubContentReferenceOutput`](crate::operation::update_hub_content_reference::UpdateHubContentReferenceOutput).
    pub fn builder() -> crate::operation::update_hub_content_reference::builders::UpdateHubContentReferenceOutputBuilder {
        crate::operation::update_hub_content_reference::builders::UpdateHubContentReferenceOutputBuilder::default()
    }
}

/// A builder for [`UpdateHubContentReferenceOutput`](crate::operation::update_hub_content_reference::UpdateHubContentReferenceOutput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct UpdateHubContentReferenceOutputBuilder {
    pub(crate) hub_arn: ::std::option::Option<::std::string::String>,
    pub(crate) hub_content_arn: ::std::option::Option<::std::string::String>,
    _request_id: Option<String>,
}
impl UpdateHubContentReferenceOutputBuilder {
    /// <p>The ARN of the private model hub that contains the updated hub content.</p>
    /// This field is required.
    pub fn hub_arn(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.hub_arn = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The ARN of the private model hub that contains the updated hub content.</p>
    pub fn set_hub_arn(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.hub_arn = input;
        self
    }
    /// <p>The ARN of the private model hub that contains the updated hub content.</p>
    pub fn get_hub_arn(&self) -> &::std::option::Option<::std::string::String> {
        &self.hub_arn
    }
    /// <p>The ARN of the hub content resource that was updated.</p>
    /// This field is required.
    pub fn hub_content_arn(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.hub_content_arn = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The ARN of the hub content resource that was updated.</p>
    pub fn set_hub_content_arn(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.hub_content_arn = input;
        self
    }
    /// <p>The ARN of the hub content resource that was updated.</p>
    pub fn get_hub_content_arn(&self) -> &::std::option::Option<::std::string::String> {
        &self.hub_content_arn
    }
    pub(crate) fn _request_id(mut self, request_id: impl Into<String>) -> Self {
        self._request_id = Some(request_id.into());
        self
    }

    pub(crate) fn _set_request_id(&mut self, request_id: Option<String>) -> &mut Self {
        self._request_id = request_id;
        self
    }
    /// Consumes the builder and constructs a [`UpdateHubContentReferenceOutput`](crate::operation::update_hub_content_reference::UpdateHubContentReferenceOutput).
    pub fn build(self) -> crate::operation::update_hub_content_reference::UpdateHubContentReferenceOutput {
        crate::operation::update_hub_content_reference::UpdateHubContentReferenceOutput {
            hub_arn: self.hub_arn,
            hub_content_arn: self.hub_content_arn,
            _request_id: self._request_id,
        }
    }
}
