...
/// When writing a match expression against `AdditionalS3DataSourceDataType`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `additionals3datasourcedatatype` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `AdditionalS3DataSourceDataType::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `AdditionalS3DataSourceDataType::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `AdditionalS3DataSourceDataType::NewFeature` is defined.
/// Specifically, when `additionals3datasourcedatatype` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `AdditionalS3DataSourceDataType::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for AdditionalS3DataSourceDataType {
    fn from(s: &str) -> Self {
        match s {
...
impl ::std::str::FromStr for AdditionalS3DataSourceDataType {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(AdditionalS3DataSourceDataType::from(s))
    }
}
impl AdditionalS3DataSourceDataType {
