...
/// When writing a match expression against `RoutingStrategy`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `routingstrategy` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `RoutingStrategy::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `RoutingStrategy::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `RoutingStrategy::NewFeature` is defined.
/// Specifically, when `routingstrategy` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `RoutingStrategy::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for RoutingStrategy {
    fn from(s: &str) -> Self {
        match s {
...
impl ::std::str::FromStr for RoutingStrategy {
    ...
    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(RoutingStrategy::from(s))
    }
}
