// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[cfg_attr(feature = "serde-serialize", derive(::serde::Serialize))]
#[cfg_attr(feature = "serde-deserialize", derive(::serde::Deserialize))]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct CreateOptionGroupOutput {
    /// <p></p>
    pub option_group: ::std::option::Option<crate::types::OptionGroup>,
    _request_id: Option<String>,
}
impl CreateOptionGroupOutput {
    /// <p></p>
    pub fn option_group(&self) -> ::std::option::Option<&crate::types::OptionGroup> {
        self.option_group.as_ref()
    }
}
impl ::aws_types::request_id::RequestId for CreateOptionGroupOutput {
    fn request_id(&self) -> Option<&str> {
        self._request_id.as_deref()
    }
}
impl CreateOptionGroupOutput {
    /// Creates a new builder-style object to manufacture [`CreateOptionGroupOutput`](crate::operation::create_option_group::CreateOptionGroupOutput).
    pub fn builder() -> crate::operation::create_option_group::builders::CreateOptionGroupOutputBuilder {
        crate::operation::create_option_group::builders::CreateOptionGroupOutputBuilder::default()
    }
}

/// A builder for [`CreateOptionGroupOutput`](crate::operation::create_option_group::CreateOptionGroupOutput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct CreateOptionGroupOutputBuilder {
    pub(crate) option_group: ::std::option::Option<crate::types::OptionGroup>,
    _request_id: Option<String>,
}
impl CreateOptionGroupOutputBuilder {
    /// <p></p>
    pub fn option_group(mut self, input: crate::types::OptionGroup) -> Self {
        self.option_group = ::std::option::Option::Some(input);
        self
    }
    /// <p></p>
    pub fn set_option_group(mut self, input: ::std::option::Option<crate::types::OptionGroup>) -> Self {
        self.option_group = input;
        self
    }
    /// <p></p>
    pub fn get_option_group(&self) -> &::std::option::Option<crate::types::OptionGroup> {
        &self.option_group
    }
    pub(crate) fn _request_id(mut self, request_id: impl Into<String>) -> Self {
        self._request_id = Some(request_id.into());
        self
    }

    pub(crate) fn _set_request_id(&mut self, request_id: Option<String>) -> &mut Self {
        self._request_id = request_id;
        self
    }
    /// Consumes the builder and constructs a [`CreateOptionGroupOutput`](crate::operation::create_option_group::CreateOptionGroupOutput).
    pub fn build(self) -> crate::operation::create_option_group::CreateOptionGroupOutput {
        crate::operation::create_option_group::CreateOptionGroupOutput {
            option_group: self.option_group,
            _request_id: self._request_id,
        }
    }
}
