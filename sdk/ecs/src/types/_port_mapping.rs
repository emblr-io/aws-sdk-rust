...
/// <p>If you use containers in a task with the <code>awsvpc</code> or <code>host</code> network mode, specify the exposed ports using <code>containerPort</code>. The <code>hostPort</code> can be left blank or it must be the same value as the <code>containerPort</code>.</p>
/// <p>Most fields of this parameter (<code>containerPort</code>, <code>hostPort</code>, <code>protocol</code>) maps to <code>PortBindings</code> in the docker container create command and the <code>--publish</code> option to <code>docker run</code>. If the network mode of a task definition is set to <code>host</code>, host ports must either be undefined or match the container port in the port mapping.</p><note>
/// <p>You can't expose the same container port for multiple protocols. If you attempt this, an error is returned.</p>
/// </note>
/// <p>After a task reaches the <code>RUNNING</code> status, manual and automatic host and container port assignments are visible in the <code>networkBindings</code> section of <a href="https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_DescribeTasks.html">DescribeTasks</a> API responses.</p>
#[non_exhaustive]
...
pub struct PortMapping {
    ...
    /// <ul>
    /// <li>
    /// <p>For containers in a task with the <code>awsvpc</code> network mode, the <code>hostPort</code> is set to the same value as the <code>containerPort</code>. This is a static mapping strategy.</p></li>
    /// <li>
    /// <p>For containers in a task with the <code>bridge</code> network mode, the Amazon ECS agent finds open ports on the host and automatically binds them to the container ports. This is a dynamic mapping strategy.</p></li>
    /// </ul>
    ...
    /// <p><code>appProtocol</code> is immutable in a Service Connect service. Updating this field requires a service deletion and redeployment.</p>
    ...
    /// <ul>
    /// <li>
    /// <p>You must use either the <code>bridge</code> network mode or the <code>awsvpc</code> network mode.</p></li>
    /// <li>
    /// <p>This parameter is available for both the EC2 and Fargate launch types.</p></li>
    /// <li>
    /// <p>This parameter is available for both the Linux and Windows operating systems.</p></li>
    /// <li>
    /// <p>The container instance must have at least version 1.67.0 of the container agent and at least version 1.67.0-1 of the <code>ecs-init</code> package</p></li>
    /// <li>
    /// <p>You can specify a maximum of 100 port ranges per container.</p></li>
    /// <li>
    /// <p>You do not specify a <code>hostPortRange</code>. The value of the <code>hostPortRange</code> is set as follows:</p>
    /// <ul>
    /// <li>
    /// <p>For containers in a task with the <code>awsvpc</code> network mode, the <code>hostPortRange</code> is set to the same value as the <code>containerPortRange</code>. This is a static mapping strategy.</p></li>
    /// <li>
    /// <p>For containers in a task with the <code>bridge</code> network mode, the Amazon ECS agent finds open host ports from the default ephemeral range and passes it to docker to bind them to the container ports.</p></li>
    /// </ul></li>
    /// <li>
    /// <p>The <code>containerPortRange</code> valid values are between 1 and 65535.</p></li>
    /// <li>
    /// <p>A port can only be included in one port mapping per container.</p></li>
    /// <li>
    /// <p>You cannot specify overlapping port ranges.</p></li>
    /// <li>
    /// <p>The first port in the range must be less than last port in the range.</p></li>
    /// <li>
...
impl PortMappingBuilder {
    ...
    pub fn app_protocol(mut self, input: crate::types::ApplicationProtocol) -> Self {
        self.app_protocol = ::std::option::Option::Some(input);
        self
    }
    /// <p>The application protocol that's used for the port mapping. This parameter only applies to Service Connect. We recommend that you set this parameter to be consistent with the protocol that your application uses. If you set this parameter, Amazon ECS adds protocol-specific connection handling to the Service Connect proxy. If you set this parameter, Amazon ECS adds protocol-specific telemetry in the Amazon ECS console and CloudWatch.</p>
    /// <p>If you don't set a value for this parameter, then TCP is used. However, Amazon ECS doesn't add protocol-specific telemetry for TCP.</p>
    ...
    /// <p><code>appProtocol</code> is immutable in a Service Connect service. Updating this field requires a service deletion and redeployment.</p>
    /// <p>Tasks that run in a namespace can use short names to connect to services in the namespace. Tasks can connect to services across all of the clusters in the namespace. Tasks connect through a managed proxy container that collects logs and metrics for increased visibility. Only the tasks that Amazon ECS services create are supported with Service Connect. For more information, see <a href="https://docs.aws.amazon.com/AmazonECS/latest/developerguide/service-connect.html">Service Connect</a> in the <i>Amazon Elastic Container Service Developer Guide</i>.</p>
    pub fn get_app_protocol(&self) -> &::std::option::Option<crate::types::ApplicationProtocol> {
        &self.app_protocol
    }
    /// <p>The port number range on the container that's bound to the dynamically mapped host port range.</p>
    /// <p>The following rules apply when you specify a <code>containerPortRange</code>:</p>
    ...
    /// </ul>
    /// <p>You can call <a href="https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_DescribeTasks.html"> <code>DescribeTasks</code> </a> to view the <code>hostPortRange</code> which are the host ports that are bound to the container ports.</p>
    pub fn get_container_port_range(&self) -> &::std::option::Option<::std::string::String> {
