...
/// When writing a match expression against `PivotTableDataPathType`, it is important to ensure
...
///     PivotTableDataPathType::CountMetricColumn => { /* ... */ },
///     PivotTableDataPathType::EmptyColumnHeader => { /* ... */ },
///     PivotTableDataPathType::HierarchyRowsLayoutColumn => { /* ... */ },
///     PivotTableDataPathType::MultipleRowMetricsColumn => { /* ... */ },
///     other @ _ if other.as_str() == "NewFeature" => { /* handles a case for `NewFeature` */ },
///     _ => { /* ... */ },
/// }
/// ```
/// The above code demonstrates that when `pivottabledatapathtype` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `PivotTableDataPathType::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `PivotTableDataPathType::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `PivotTableDataPathType::NewFeature` is defined.
