...
impl OperandOne {
    ...
    /// Tries to convert the enum instance into [`FieldId`](crate::types::OperandOne::FieldId), extracting the inner [`String`](::std::string::String).
    /// Returns `Err(&Self)` if it can't be converted.
    pub fn as_field_id(&self) -> ::std::result::Result<&::std::string::String, &Self> {
        if let OperandOne::FieldId(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`FieldId`](crate::types::OperandOne::FieldId).
    pub fn is_field_id(&self) -> bool {
        self.as_field_id().is_ok()
    }
