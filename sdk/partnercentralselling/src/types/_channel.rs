// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// When writing a match expression against `Channel`, it is important to ensure
/// your code is forward-compatible. That is, if a match arm handles a case for a
/// feature that is supported by the service but has not been represented as an enum
/// variant in a current version of SDK, your code should continue to work when you
/// upgrade SDK to a future version in which the enum does include a variant for that
/// feature.
///
/// Here is an example of how you can make a match expression forward-compatible:
///
/// ```text
/// # let channel = unimplemented!();
/// match channel {
///     Channel::AwsMarketingCentral => { /* ... */ },
///     Channel::ContentSyndication => { /* ... */ },
///     Channel::Display => { /* ... */ },
///     Channel::Email => { /* ... */ },
///     Channel::LiveEvent => { /* ... */ },
///     Channel::OutOfHome => { /* ... */ },
///     Channel::Print => { /* ... */ },
///     Channel::Search => { /* ... */ },
///     Channel::Social => { /* ... */ },
///     Channel::Tv => { /* ... */ },
///     Channel::Telemarketing => { /* ... */ },
///     Channel::Video => { /* ... */ },
///     Channel::VirtualEvent => { /* ... */ },
///     other @ _ if other.as_str() == "NewFeature" => { /* handles a case for `NewFeature` */ },
///     _ => { /* ... */ },
/// }
/// ```
/// The above code demonstrates that when `channel` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `Channel::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `Channel::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `Channel::NewFeature` is defined.
/// Specifically, when `channel` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `Channel::NewFeature` also yielding `"NewFeature"`.
///
/// Explicitly matching on the `Unknown` variant should
/// be avoided for two reasons:
/// - The inner data `UnknownVariantValue` is opaque, and no further information can be extracted.
/// - It might inadvertently shadow other intended match arms.
///
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[cfg_attr(feature = "serde-serialize", derive(::serde::Serialize))]
#[cfg_attr(feature = "serde-deserialize", derive(::serde::Deserialize))]
#[derive(
    ::std::clone::Clone, ::std::cmp::Eq, ::std::cmp::Ord, ::std::cmp::PartialEq, ::std::cmp::PartialOrd, ::std::fmt::Debug, ::std::hash::Hash,
)]
pub enum Channel {
    #[allow(missing_docs)] // documentation missing in model
    AwsMarketingCentral,
    #[allow(missing_docs)] // documentation missing in model
    ContentSyndication,
    #[allow(missing_docs)] // documentation missing in model
    Display,
    #[allow(missing_docs)] // documentation missing in model
    Email,
    #[allow(missing_docs)] // documentation missing in model
    LiveEvent,
    #[allow(missing_docs)] // documentation missing in model
    OutOfHome,
    #[allow(missing_docs)] // documentation missing in model
    Print,
    #[allow(missing_docs)] // documentation missing in model
    Search,
    #[allow(missing_docs)] // documentation missing in model
    Social,
    #[allow(missing_docs)] // documentation missing in model
    Tv,
    #[allow(missing_docs)] // documentation missing in model
    Telemarketing,
    #[allow(missing_docs)] // documentation missing in model
    Video,
    #[allow(missing_docs)] // documentation missing in model
    VirtualEvent,
    /// `Unknown` contains new variants that have been added since this code was generated.
    #[deprecated(note = "Don't directly match on `Unknown`. See the docs on this enum for the correct way to handle unknown variants.")]
    Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue),
}
impl ::std::convert::From<&str> for Channel {
    fn from(s: &str) -> Self {
        match s {
            "AWS Marketing Central" => Channel::AwsMarketingCentral,
            "Content Syndication" => Channel::ContentSyndication,
            "Display" => Channel::Display,
            "Email" => Channel::Email,
            "Live Event" => Channel::LiveEvent,
            "Out Of Home (OOH)" => Channel::OutOfHome,
            "Print" => Channel::Print,
            "Search" => Channel::Search,
            "Social" => Channel::Social,
            "TV" => Channel::Tv,
            "Telemarketing" => Channel::Telemarketing,
            "Video" => Channel::Video,
            "Virtual Event" => Channel::VirtualEvent,
            other => Channel::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
impl ::std::str::FromStr for Channel {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(Channel::from(s))
    }
}
impl Channel {
    /// Returns the `&str` value of the enum member.
    pub fn as_str(&self) -> &str {
        match self {
            Channel::AwsMarketingCentral => "AWS Marketing Central",
            Channel::ContentSyndication => "Content Syndication",
            Channel::Display => "Display",
            Channel::Email => "Email",
            Channel::LiveEvent => "Live Event",
            Channel::OutOfHome => "Out Of Home (OOH)",
            Channel::Print => "Print",
            Channel::Search => "Search",
            Channel::Social => "Social",
            Channel::Tv => "TV",
            Channel::Telemarketing => "Telemarketing",
            Channel::Video => "Video",
            Channel::VirtualEvent => "Virtual Event",
            Channel::Unknown(value) => value.as_str(),
        }
    }
    /// Returns all the `&str` representations of the enum members.
    pub const fn values() -> &'static [&'static str] {
        &[
            "AWS Marketing Central",
            "Content Syndication",
            "Display",
            "Email",
            "Live Event",
            "Out Of Home (OOH)",
            "Print",
            "Search",
            "Social",
            "TV",
            "Telemarketing",
            "Video",
            "Virtual Event",
        ]
    }
}
impl ::std::convert::AsRef<str> for Channel {
    fn as_ref(&self) -> &str {
        self.as_str()
    }
}
impl Channel {
    /// Parses the enum value while disallowing unknown variants.
    ///
    /// Unknown variants will result in an error.
    pub fn try_parse(value: &str) -> ::std::result::Result<Self, crate::error::UnknownVariantError> {
        match Self::from(value) {
            #[allow(deprecated)]
            Self::Unknown(_) => ::std::result::Result::Err(crate::error::UnknownVariantError::new(value)),
            known => Ok(known),
        }
    }
}
impl ::std::fmt::Display for Channel {
    fn fmt(&self, f: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
        match self {
            Channel::AwsMarketingCentral => write!(f, "AWS Marketing Central"),
            Channel::ContentSyndication => write!(f, "Content Syndication"),
            Channel::Display => write!(f, "Display"),
            Channel::Email => write!(f, "Email"),
            Channel::LiveEvent => write!(f, "Live Event"),
            Channel::OutOfHome => write!(f, "Out Of Home (OOH)"),
            Channel::Print => write!(f, "Print"),
            Channel::Search => write!(f, "Search"),
            Channel::Social => write!(f, "Social"),
            Channel::Tv => write!(f, "TV"),
            Channel::Telemarketing => write!(f, "Telemarketing"),
            Channel::Video => write!(f, "Video"),
            Channel::VirtualEvent => write!(f, "Virtual Event"),
            Channel::Unknown(value) => write!(f, "{}", value),
        }
    }
}
