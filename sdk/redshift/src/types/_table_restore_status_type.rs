...
/// When writing a match expression against `TableRestoreStatusType`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `tablerestorestatustype` represents
...
/// version of SDK where the variant `TableRestoreStatusType::NewFeature` is defined.
...
impl ::std::convert::From<&str> for TableRestoreStatusType {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => TableRestoreStatusType::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
