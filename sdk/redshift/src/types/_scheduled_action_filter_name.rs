...
/// When writing a match expression against `ScheduledActionFilterName`, it is important to ensure
...
///     ScheduledActionFilterName::ClusterIdentifier => { /* ... */ },
///     ScheduledActionFilterName::IamRole => { /* ... */ },
///     other @ _ if other.as_str() == "NewFeature" => { /* handles a case for `NewFeature` */ },
///     _ => { /* ... */ },
/// }
/// ```
/// The above code demonstrates that when `scheduledactionfiltername` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `ScheduledActionFilterName::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `ScheduledActionFilterName::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `ScheduledActionFilterName::NewFeature` is defined.
...
impl ::std::convert::From<&str> for ScheduledActionFilterName {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => ScheduledActionFilterName::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
