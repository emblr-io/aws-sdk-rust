// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>The details of a queue member.</p>
#[non_exhaustive]
#[cfg_attr(feature = "serde-serialize", derive(::serde::Serialize))]
#[cfg_attr(feature = "serde-deserialize", derive(::serde::Deserialize))]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct QueueMember {
    /// <p>The farm ID.</p>
    pub farm_id: ::std::string::String,
    /// <p>The queue ID.</p>
    pub queue_id: ::std::string::String,
    /// <p>The principal ID of the queue member.</p>
    pub principal_id: ::std::string::String,
    /// <p>The principal type of the queue member.</p>
    pub principal_type: crate::types::DeadlinePrincipalType,
    /// <p>The identity store ID.</p>
    pub identity_store_id: ::std::string::String,
    /// <p>The queue member's membership level.</p>
    pub membership_level: crate::types::MembershipLevel,
}
impl QueueMember {
    /// <p>The farm ID.</p>
    pub fn farm_id(&self) -> &str {
        use std::ops::Deref;
        self.farm_id.deref()
    }
    /// <p>The queue ID.</p>
    pub fn queue_id(&self) -> &str {
        use std::ops::Deref;
        self.queue_id.deref()
    }
    /// <p>The principal ID of the queue member.</p>
    pub fn principal_id(&self) -> &str {
        use std::ops::Deref;
        self.principal_id.deref()
    }
    /// <p>The principal type of the queue member.</p>
    pub fn principal_type(&self) -> &crate::types::DeadlinePrincipalType {
        &self.principal_type
    }
    /// <p>The identity store ID.</p>
    pub fn identity_store_id(&self) -> &str {
        use std::ops::Deref;
        self.identity_store_id.deref()
    }
    /// <p>The queue member's membership level.</p>
    pub fn membership_level(&self) -> &crate::types::MembershipLevel {
        &self.membership_level
    }
}
impl QueueMember {
    /// Creates a new builder-style object to manufacture [`QueueMember`](crate::types::QueueMember).
    pub fn builder() -> crate::types::builders::QueueMemberBuilder {
        crate::types::builders::QueueMemberBuilder::default()
    }
}

/// A builder for [`QueueMember`](crate::types::QueueMember).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct QueueMemberBuilder {
    pub(crate) farm_id: ::std::option::Option<::std::string::String>,
    pub(crate) queue_id: ::std::option::Option<::std::string::String>,
    pub(crate) principal_id: ::std::option::Option<::std::string::String>,
    pub(crate) principal_type: ::std::option::Option<crate::types::DeadlinePrincipalType>,
    pub(crate) identity_store_id: ::std::option::Option<::std::string::String>,
    pub(crate) membership_level: ::std::option::Option<crate::types::MembershipLevel>,
}
impl QueueMemberBuilder {
    /// <p>The farm ID.</p>
    /// This field is required.
    pub fn farm_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.farm_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The farm ID.</p>
    pub fn set_farm_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.farm_id = input;
        self
    }
    /// <p>The farm ID.</p>
    pub fn get_farm_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.farm_id
    }
    /// <p>The queue ID.</p>
    /// This field is required.
    pub fn queue_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.queue_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The queue ID.</p>
    pub fn set_queue_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.queue_id = input;
        self
    }
    /// <p>The queue ID.</p>
    pub fn get_queue_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.queue_id
    }
    /// <p>The principal ID of the queue member.</p>
    /// This field is required.
    pub fn principal_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.principal_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The principal ID of the queue member.</p>
    pub fn set_principal_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.principal_id = input;
        self
    }
    /// <p>The principal ID of the queue member.</p>
    pub fn get_principal_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.principal_id
    }
    /// <p>The principal type of the queue member.</p>
    /// This field is required.
    pub fn principal_type(mut self, input: crate::types::DeadlinePrincipalType) -> Self {
        self.principal_type = ::std::option::Option::Some(input);
        self
    }
    /// <p>The principal type of the queue member.</p>
    pub fn set_principal_type(mut self, input: ::std::option::Option<crate::types::DeadlinePrincipalType>) -> Self {
        self.principal_type = input;
        self
    }
    /// <p>The principal type of the queue member.</p>
    pub fn get_principal_type(&self) -> &::std::option::Option<crate::types::DeadlinePrincipalType> {
        &self.principal_type
    }
    /// <p>The identity store ID.</p>
    /// This field is required.
    pub fn identity_store_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.identity_store_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The identity store ID.</p>
    pub fn set_identity_store_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.identity_store_id = input;
        self
    }
    /// <p>The identity store ID.</p>
    pub fn get_identity_store_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.identity_store_id
    }
    /// <p>The queue member's membership level.</p>
    /// This field is required.
    pub fn membership_level(mut self, input: crate::types::MembershipLevel) -> Self {
        self.membership_level = ::std::option::Option::Some(input);
        self
    }
    /// <p>The queue member's membership level.</p>
    pub fn set_membership_level(mut self, input: ::std::option::Option<crate::types::MembershipLevel>) -> Self {
        self.membership_level = input;
        self
    }
    /// <p>The queue member's membership level.</p>
    pub fn get_membership_level(&self) -> &::std::option::Option<crate::types::MembershipLevel> {
        &self.membership_level
    }
    /// Consumes the builder and constructs a [`QueueMember`](crate::types::QueueMember).
    /// This method will fail if any of the following fields are not set:
    /// - [`farm_id`](crate::types::builders::QueueMemberBuilder::farm_id)
    /// - [`queue_id`](crate::types::builders::QueueMemberBuilder::queue_id)
    /// - [`principal_id`](crate::types::builders::QueueMemberBuilder::principal_id)
    /// - [`principal_type`](crate::types::builders::QueueMemberBuilder::principal_type)
    /// - [`identity_store_id`](crate::types::builders::QueueMemberBuilder::identity_store_id)
    /// - [`membership_level`](crate::types::builders::QueueMemberBuilder::membership_level)
    pub fn build(self) -> ::std::result::Result<crate::types::QueueMember, ::aws_smithy_types::error::operation::BuildError> {
        ::std::result::Result::Ok(crate::types::QueueMember {
            farm_id: self.farm_id.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "farm_id",
                    "farm_id was not specified but it is required when building QueueMember",
                )
            })?,
            queue_id: self.queue_id.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "queue_id",
                    "queue_id was not specified but it is required when building QueueMember",
                )
            })?,
            principal_id: self.principal_id.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "principal_id",
                    "principal_id was not specified but it is required when building QueueMember",
                )
            })?,
            principal_type: self.principal_type.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "principal_type",
                    "principal_type was not specified but it is required when building QueueMember",
                )
            })?,
            identity_store_id: self.identity_store_id.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "identity_store_id",
                    "identity_store_id was not specified but it is required when building QueueMember",
                )
            })?,
            membership_level: self.membership_level.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "membership_level",
                    "membership_level was not specified but it is required when building QueueMember",
                )
            })?,
        })
    }
}
