...
/// <p>Describes an Amazon GameLift Servers fleet of game hosting resources. Attributes differ based on the fleet's compute type, as follows:</p>
/// <ul>
/// <li>
/// <p>EC2 fleet attributes identify a <code>Build</code> resource (for fleets with customer game server builds) or a <code>Script</code> resource (for Amazon GameLift Servers Realtime fleets).</p></li>
/// <li>
/// <p>Amazon GameLift Servers Anywhere fleets have an abbreviated set of attributes, because most fleet configurations are set directly on the fleet's computes. Attributes include fleet identifiers and descriptive properties, creation/termination time, and fleet status.</p></li>
/// </ul>
