...
impl ApprovalStrategy {
    ...
    /// Tries to convert the enum instance into [`MofN`](crate::types::ApprovalStrategy::MofN), extracting the inner [`MofNApprovalStrategy`](crate::types::MofNApprovalStrategy).
    /// Returns `Err(&Self)` if it can't be converted.
    pub fn as_mof_n(&self) -> ::std::result::Result<&crate::types::MofNApprovalStrategy, &Self> {
        if let ApprovalStrategy::MofN(val) = &self {
            ::std::result::Result::Ok(val)
        } else {
            ::std::result::Result::Err(self)
        }
    }
    /// Returns true if this is a [`MofN`](crate::types::ApprovalStrategy::MofN).
