...
/// }
/// ```
/// The above code demonstrates that when `ondemandprovisioningallocationstrategy` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `OnDemandProvisioningAllocationStrategy::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `OnDemandProvisioningAllocationStrategy::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `OnDemandProvisioningAllocationStrategy::NewFeature` is defined.
...
impl ::std::str::FromStr for OnDemandProvisioningAllocationStrategy {
    ...
    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(OnDemandProvisioningAllocationStrategy::from(s))
    }
}
