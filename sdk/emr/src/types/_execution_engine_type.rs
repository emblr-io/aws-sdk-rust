...
/// When writing a match expression against `ExecutionEngineType`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `executionenginetype` represents
...
/// version of SDK where the variant `ExecutionEngineType::NewFeature` is defined.
...
impl ::std::convert::From<&str> for ExecutionEngineType {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => ExecutionEngineType::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
