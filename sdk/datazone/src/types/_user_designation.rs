...
/// When writing a match expression against `UserDesignation`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `userdesignation` represents
...
/// version of SDK where the variant `UserDesignation::NewFeature` is defined.
/// Specifically, when `userdesignation` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `UserDesignation::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::str::FromStr for UserDesignation {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(UserDesignation::from(s))
    }
}
impl UserDesignation {
