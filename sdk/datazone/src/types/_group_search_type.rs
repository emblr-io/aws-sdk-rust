...
/// When writing a match expression against `GroupSearchType`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `groupsearchtype` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `GroupSearchType::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `GroupSearchType::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `GroupSearchType::NewFeature` is defined.
/// Specifically, when `groupsearchtype` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `GroupSearchType::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for GroupSearchType {
    fn from(s: &str) -> Self {
        match s {
...
impl ::std::str::FromStr for GroupSearchType {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(GroupSearchType::from(s))
    }
}
impl GroupSearchType {
