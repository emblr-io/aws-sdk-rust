...
/// When writing a match expression against `TypesSearchScope`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `typessearchscope` represents
...
/// version of SDK where the variant `TypesSearchScope::NewFeature` is defined.
/// Specifically, when `typessearchscope` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `TypesSearchScope::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for TypesSearchScope {
    fn from(s: &str) -> Self {
        match s {
...
impl ::std::str::FromStr for TypesSearchScope {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(TypesSearchScope::from(s))
    }
}
impl TypesSearchScope {
