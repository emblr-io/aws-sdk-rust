...
/// When writing a match expression against `TargetEntityType`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `targetentitytype` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `TargetEntityType::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `TargetEntityType::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `TargetEntityType::NewFeature` is defined.
/// Specifically, when `targetentitytype` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `TargetEntityType::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for TargetEntityType {
    fn from(s: &str) -> Self {
        match s {
            ...
            other => TargetEntityType::Unknown(crate::primitives::sealed_enum_unknown::UnknownVariantValue(other.to_owned())),
        }
    }
}
impl ::std::str::FromStr for TargetEntityType {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(TargetEntityType::from(s))
    }
}
impl TargetEntityType {
    ...
    pub fn as_str(&self) -> &str {
        match self {
            ...
            TargetEntityType::Unknown(value) => value.as_str(),
        }
    }
