...
/// When writing a match expression against `ProjectDesignation`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `projectdesignation` represents
...
/// version of SDK where the variant `ProjectDesignation::NewFeature` is defined.
/// Specifically, when `projectdesignation` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `ProjectDesignation::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for ProjectDesignation {
    fn from(s: &str) -> Self {
        match s {
...
impl ::std::str::FromStr for ProjectDesignation {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(ProjectDesignation::from(s))
    }
}
...
impl ::std::fmt::Display for ProjectDesignation {
