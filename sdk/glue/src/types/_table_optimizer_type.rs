...
/// When writing a match expression against `TableOptimizerType`, it is important to ensure
...
/// }
/// ```
/// The above code demonstrates that when `tableoptimizertype` represents
...
/// version of SDK where the variant `TableOptimizerType::NewFeature` is defined.
/// Specifically, when `tableoptimizertype` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `TableOptimizerType::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for TableOptimizerType {
    fn from(s: &str) -> Self {
        match s {
...
impl ::std::str::FromStr for TableOptimizerType {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(TableOptimizerType::from(s))
    }
}
