// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[cfg_attr(feature = "serde-serialize", derive(::serde::Serialize))]
#[cfg_attr(feature = "serde-deserialize", derive(::serde::Deserialize))]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct StartColumnStatisticsTaskRunOutput {
    /// <p>The identifier for the column statistics task run.</p>
    pub column_statistics_task_run_id: ::std::option::Option<::std::string::String>,
    _request_id: Option<String>,
}
impl StartColumnStatisticsTaskRunOutput {
    /// <p>The identifier for the column statistics task run.</p>
    pub fn column_statistics_task_run_id(&self) -> ::std::option::Option<&str> {
        self.column_statistics_task_run_id.as_deref()
    }
}
impl ::aws_types::request_id::RequestId for StartColumnStatisticsTaskRunOutput {
    fn request_id(&self) -> Option<&str> {
        self._request_id.as_deref()
    }
}
impl StartColumnStatisticsTaskRunOutput {
    /// Creates a new builder-style object to manufacture [`StartColumnStatisticsTaskRunOutput`](crate::operation::start_column_statistics_task_run::StartColumnStatisticsTaskRunOutput).
    pub fn builder() -> crate::operation::start_column_statistics_task_run::builders::StartColumnStatisticsTaskRunOutputBuilder {
        crate::operation::start_column_statistics_task_run::builders::StartColumnStatisticsTaskRunOutputBuilder::default()
    }
}

/// A builder for [`StartColumnStatisticsTaskRunOutput`](crate::operation::start_column_statistics_task_run::StartColumnStatisticsTaskRunOutput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct StartColumnStatisticsTaskRunOutputBuilder {
    pub(crate) column_statistics_task_run_id: ::std::option::Option<::std::string::String>,
    _request_id: Option<String>,
}
impl StartColumnStatisticsTaskRunOutputBuilder {
    /// <p>The identifier for the column statistics task run.</p>
    pub fn column_statistics_task_run_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.column_statistics_task_run_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The identifier for the column statistics task run.</p>
    pub fn set_column_statistics_task_run_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.column_statistics_task_run_id = input;
        self
    }
    /// <p>The identifier for the column statistics task run.</p>
    pub fn get_column_statistics_task_run_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.column_statistics_task_run_id
    }
    pub(crate) fn _request_id(mut self, request_id: impl Into<String>) -> Self {
        self._request_id = Some(request_id.into());
        self
    }

    pub(crate) fn _set_request_id(&mut self, request_id: Option<String>) -> &mut Self {
        self._request_id = request_id;
        self
    }
    /// Consumes the builder and constructs a [`StartColumnStatisticsTaskRunOutput`](crate::operation::start_column_statistics_task_run::StartColumnStatisticsTaskRunOutput).
    pub fn build(self) -> crate::operation::start_column_statistics_task_run::StartColumnStatisticsTaskRunOutput {
        crate::operation::start_column_statistics_task_run::StartColumnStatisticsTaskRunOutput {
            column_statistics_task_run_id: self.column_statistics_task_run_id,
            _request_id: self._request_id,
        }
    }
}
