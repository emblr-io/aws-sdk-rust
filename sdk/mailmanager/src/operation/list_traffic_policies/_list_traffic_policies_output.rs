...
impl ListTrafficPoliciesOutput {
    /// <p>The list of traffic policies.</p>
    ///
    /// If no value was sent for this field, a default will be set. If you want to determine if no value was sent, use `.traffic_policies.is_none()`.
    pub fn traffic_policies(&self) -> &[crate::types::TrafficPolicy] {
        self.traffic_policies.as_deref().unwrap_or_default()
    }
    /// <p>If NextToken is returned, there are more results available. The value of NextToken is a unique pagination token for each page. Make the call again using the returned token to retrieve the next page.</p>
