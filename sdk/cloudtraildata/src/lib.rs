...
/// let config = aws_sdk_cloudtraildata::config::Builder::from(&sdk_config)
/// # /*
...
/// For example, the [`PutAuditEvents`](crate::operation::put_audit_events) operation has
/// a [`Client::put_audit_events`], function which returns a builder for that operation.
/// The fluent builder ultimately has a `send()` function that returns an async future that
/// returns a result, as illustrated below:
///
/// ```rust,ignore
/// let result = client.put_audit_events()
///     .channel_arn("example")
///     .send()
///     .await;
/// ```
