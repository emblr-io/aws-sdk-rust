...
/// When writing a match expression against `UnprocessedErrorCode`, it is important to ensure
...
///
/// ```text
/// # let unprocessederrorcode = unimplemented!();
/// match unprocessederrorcode {
///     UnprocessedErrorCode::AccessDenied => { /* ... */ },
...
/// }
/// ```
/// The above code demonstrates that when `unprocessederrorcode` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `UnprocessedErrorCode::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `UnprocessedErrorCode::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `UnprocessedErrorCode::NewFeature` is defined.
/// Specifically, when `unprocessederrorcode` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `UnprocessedErrorCode::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for UnprocessedErrorCode {
    fn from(s: &str) -> Self {
        match s {
            "ACCESS_DENIED" => UnprocessedErrorCode::AccessDenied,
...
impl ::std::str::FromStr for UnprocessedErrorCode {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(UnprocessedErrorCode::from(s))
    }
}
impl UnprocessedErrorCode {
    /// Returns the `&str` value of the enum member.
    pub fn as_str(&self) -> &str {
        match self {
            UnprocessedErrorCode::AccessDenied => "ACCESS_DENIED",
