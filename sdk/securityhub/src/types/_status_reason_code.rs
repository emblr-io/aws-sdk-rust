...
///
/// ```text
/// # let statusreasoncode = unimplemented!();
/// match statusreasoncode {
...
/// }
/// ```
/// The above code demonstrates that when `statusreasoncode` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `StatusReasonCode::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `StatusReasonCode::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `StatusReasonCode::NewFeature` is defined.
/// Specifically, when `statusreasoncode` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `StatusReasonCode::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::str::FromStr for StatusReasonCode {
    ...
    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(StatusReasonCode::from(s))
    }
}
...
impl ::std::convert::AsRef<str> for StatusReasonCode {
    fn as_ref(&self) -> &str {
        self.as_str()
    }
}
