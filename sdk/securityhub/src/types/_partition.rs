...
/// }
/// ```
/// The above code demonstrates that when `partition` represents
...
/// version of SDK where the variant `Partition::NewFeature` is defined.
/// Specifically, when `partition` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `Partition::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::str::FromStr for Partition {
    ...
    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(Partition::from(s))
    }
}
impl Partition {
