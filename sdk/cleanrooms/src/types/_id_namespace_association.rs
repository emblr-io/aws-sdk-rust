// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.

/// <p>Provides information to create the ID namespace association.</p>
#[non_exhaustive]
#[cfg_attr(feature = "serde-serialize", derive(::serde::Serialize))]
#[cfg_attr(feature = "serde-deserialize", derive(::serde::Deserialize))]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct IdNamespaceAssociation {
    /// <p>The unique identifier for this ID namespace association.</p>
    pub id: ::std::string::String,
    /// <p>The Amazon Resource Name (ARN) of the ID namespace association.</p>
    pub arn: ::std::string::String,
    /// <p>The unique identifier of the membership resource for this ID namespace association.</p>
    pub membership_id: ::std::string::String,
    /// <p>The Amazon Resource Name (ARN) of the membership resource for this ID namespace association.</p>
    pub membership_arn: ::std::string::String,
    /// <p>The unique identifier of the collaboration that contains this ID namespace association.</p>
    pub collaboration_id: ::std::string::String,
    /// <p>The Amazon Resource Name (ARN) of the collaboration that contains this ID namespace association.</p>
    pub collaboration_arn: ::std::string::String,
    /// <p>The name of this ID namespace association.</p>
    pub name: ::std::string::String,
    /// <p>The description of the ID namespace association.</p>
    pub description: ::std::option::Option<::std::string::String>,
    /// <p>The time at which the ID namespace association was created.</p>
    pub create_time: ::aws_smithy_types::DateTime,
    /// <p>The most recent time at which the ID namespace association was updated.</p>
    pub update_time: ::aws_smithy_types::DateTime,
    /// <p>The input reference configuration for the ID namespace association.</p>
    pub input_reference_config: ::std::option::Option<crate::types::IdNamespaceAssociationInputReferenceConfig>,
    /// <p>The input reference properties for the ID namespace association.</p>
    pub input_reference_properties: ::std::option::Option<crate::types::IdNamespaceAssociationInputReferenceProperties>,
    /// <p>The configuration settings for the ID mapping table.</p>
    pub id_mapping_config: ::std::option::Option<crate::types::IdMappingConfig>,
}
impl IdNamespaceAssociation {
    /// <p>The unique identifier for this ID namespace association.</p>
    pub fn id(&self) -> &str {
        use std::ops::Deref;
        self.id.deref()
    }
    /// <p>The Amazon Resource Name (ARN) of the ID namespace association.</p>
    pub fn arn(&self) -> &str {
        use std::ops::Deref;
        self.arn.deref()
    }
    /// <p>The unique identifier of the membership resource for this ID namespace association.</p>
    pub fn membership_id(&self) -> &str {
        use std::ops::Deref;
        self.membership_id.deref()
    }
    /// <p>The Amazon Resource Name (ARN) of the membership resource for this ID namespace association.</p>
    pub fn membership_arn(&self) -> &str {
        use std::ops::Deref;
        self.membership_arn.deref()
    }
    /// <p>The unique identifier of the collaboration that contains this ID namespace association.</p>
    pub fn collaboration_id(&self) -> &str {
        use std::ops::Deref;
        self.collaboration_id.deref()
    }
    /// <p>The Amazon Resource Name (ARN) of the collaboration that contains this ID namespace association.</p>
    pub fn collaboration_arn(&self) -> &str {
        use std::ops::Deref;
        self.collaboration_arn.deref()
    }
    /// <p>The name of this ID namespace association.</p>
    pub fn name(&self) -> &str {
        use std::ops::Deref;
        self.name.deref()
    }
    /// <p>The description of the ID namespace association.</p>
    pub fn description(&self) -> ::std::option::Option<&str> {
        self.description.as_deref()
    }
    /// <p>The time at which the ID namespace association was created.</p>
    pub fn create_time(&self) -> &::aws_smithy_types::DateTime {
        &self.create_time
    }
    /// <p>The most recent time at which the ID namespace association was updated.</p>
    pub fn update_time(&self) -> &::aws_smithy_types::DateTime {
        &self.update_time
    }
    /// <p>The input reference configuration for the ID namespace association.</p>
    pub fn input_reference_config(&self) -> ::std::option::Option<&crate::types::IdNamespaceAssociationInputReferenceConfig> {
        self.input_reference_config.as_ref()
    }
    /// <p>The input reference properties for the ID namespace association.</p>
    pub fn input_reference_properties(&self) -> ::std::option::Option<&crate::types::IdNamespaceAssociationInputReferenceProperties> {
        self.input_reference_properties.as_ref()
    }
    /// <p>The configuration settings for the ID mapping table.</p>
    pub fn id_mapping_config(&self) -> ::std::option::Option<&crate::types::IdMappingConfig> {
        self.id_mapping_config.as_ref()
    }
}
impl IdNamespaceAssociation {
    /// Creates a new builder-style object to manufacture [`IdNamespaceAssociation`](crate::types::IdNamespaceAssociation).
    pub fn builder() -> crate::types::builders::IdNamespaceAssociationBuilder {
        crate::types::builders::IdNamespaceAssociationBuilder::default()
    }
}

/// A builder for [`IdNamespaceAssociation`](crate::types::IdNamespaceAssociation).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct IdNamespaceAssociationBuilder {
    pub(crate) id: ::std::option::Option<::std::string::String>,
    pub(crate) arn: ::std::option::Option<::std::string::String>,
    pub(crate) membership_id: ::std::option::Option<::std::string::String>,
    pub(crate) membership_arn: ::std::option::Option<::std::string::String>,
    pub(crate) collaboration_id: ::std::option::Option<::std::string::String>,
    pub(crate) collaboration_arn: ::std::option::Option<::std::string::String>,
    pub(crate) name: ::std::option::Option<::std::string::String>,
    pub(crate) description: ::std::option::Option<::std::string::String>,
    pub(crate) create_time: ::std::option::Option<::aws_smithy_types::DateTime>,
    pub(crate) update_time: ::std::option::Option<::aws_smithy_types::DateTime>,
    pub(crate) input_reference_config: ::std::option::Option<crate::types::IdNamespaceAssociationInputReferenceConfig>,
    pub(crate) input_reference_properties: ::std::option::Option<crate::types::IdNamespaceAssociationInputReferenceProperties>,
    pub(crate) id_mapping_config: ::std::option::Option<crate::types::IdMappingConfig>,
}
impl IdNamespaceAssociationBuilder {
    /// <p>The unique identifier for this ID namespace association.</p>
    /// This field is required.
    pub fn id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The unique identifier for this ID namespace association.</p>
    pub fn set_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.id = input;
        self
    }
    /// <p>The unique identifier for this ID namespace association.</p>
    pub fn get_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.id
    }
    /// <p>The Amazon Resource Name (ARN) of the ID namespace association.</p>
    /// This field is required.
    pub fn arn(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.arn = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The Amazon Resource Name (ARN) of the ID namespace association.</p>
    pub fn set_arn(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.arn = input;
        self
    }
    /// <p>The Amazon Resource Name (ARN) of the ID namespace association.</p>
    pub fn get_arn(&self) -> &::std::option::Option<::std::string::String> {
        &self.arn
    }
    /// <p>The unique identifier of the membership resource for this ID namespace association.</p>
    /// This field is required.
    pub fn membership_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.membership_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The unique identifier of the membership resource for this ID namespace association.</p>
    pub fn set_membership_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.membership_id = input;
        self
    }
    /// <p>The unique identifier of the membership resource for this ID namespace association.</p>
    pub fn get_membership_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.membership_id
    }
    /// <p>The Amazon Resource Name (ARN) of the membership resource for this ID namespace association.</p>
    /// This field is required.
    pub fn membership_arn(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.membership_arn = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The Amazon Resource Name (ARN) of the membership resource for this ID namespace association.</p>
    pub fn set_membership_arn(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.membership_arn = input;
        self
    }
    /// <p>The Amazon Resource Name (ARN) of the membership resource for this ID namespace association.</p>
    pub fn get_membership_arn(&self) -> &::std::option::Option<::std::string::String> {
        &self.membership_arn
    }
    /// <p>The unique identifier of the collaboration that contains this ID namespace association.</p>
    /// This field is required.
    pub fn collaboration_id(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.collaboration_id = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The unique identifier of the collaboration that contains this ID namespace association.</p>
    pub fn set_collaboration_id(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.collaboration_id = input;
        self
    }
    /// <p>The unique identifier of the collaboration that contains this ID namespace association.</p>
    pub fn get_collaboration_id(&self) -> &::std::option::Option<::std::string::String> {
        &self.collaboration_id
    }
    /// <p>The Amazon Resource Name (ARN) of the collaboration that contains this ID namespace association.</p>
    /// This field is required.
    pub fn collaboration_arn(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.collaboration_arn = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The Amazon Resource Name (ARN) of the collaboration that contains this ID namespace association.</p>
    pub fn set_collaboration_arn(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.collaboration_arn = input;
        self
    }
    /// <p>The Amazon Resource Name (ARN) of the collaboration that contains this ID namespace association.</p>
    pub fn get_collaboration_arn(&self) -> &::std::option::Option<::std::string::String> {
        &self.collaboration_arn
    }
    /// <p>The name of this ID namespace association.</p>
    /// This field is required.
    pub fn name(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.name = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The name of this ID namespace association.</p>
    pub fn set_name(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.name = input;
        self
    }
    /// <p>The name of this ID namespace association.</p>
    pub fn get_name(&self) -> &::std::option::Option<::std::string::String> {
        &self.name
    }
    /// <p>The description of the ID namespace association.</p>
    pub fn description(mut self, input: impl ::std::convert::Into<::std::string::String>) -> Self {
        self.description = ::std::option::Option::Some(input.into());
        self
    }
    /// <p>The description of the ID namespace association.</p>
    pub fn set_description(mut self, input: ::std::option::Option<::std::string::String>) -> Self {
        self.description = input;
        self
    }
    /// <p>The description of the ID namespace association.</p>
    pub fn get_description(&self) -> &::std::option::Option<::std::string::String> {
        &self.description
    }
    /// <p>The time at which the ID namespace association was created.</p>
    /// This field is required.
    pub fn create_time(mut self, input: ::aws_smithy_types::DateTime) -> Self {
        self.create_time = ::std::option::Option::Some(input);
        self
    }
    /// <p>The time at which the ID namespace association was created.</p>
    pub fn set_create_time(mut self, input: ::std::option::Option<::aws_smithy_types::DateTime>) -> Self {
        self.create_time = input;
        self
    }
    /// <p>The time at which the ID namespace association was created.</p>
    pub fn get_create_time(&self) -> &::std::option::Option<::aws_smithy_types::DateTime> {
        &self.create_time
    }
    /// <p>The most recent time at which the ID namespace association was updated.</p>
    /// This field is required.
    pub fn update_time(mut self, input: ::aws_smithy_types::DateTime) -> Self {
        self.update_time = ::std::option::Option::Some(input);
        self
    }
    /// <p>The most recent time at which the ID namespace association was updated.</p>
    pub fn set_update_time(mut self, input: ::std::option::Option<::aws_smithy_types::DateTime>) -> Self {
        self.update_time = input;
        self
    }
    /// <p>The most recent time at which the ID namespace association was updated.</p>
    pub fn get_update_time(&self) -> &::std::option::Option<::aws_smithy_types::DateTime> {
        &self.update_time
    }
    /// <p>The input reference configuration for the ID namespace association.</p>
    /// This field is required.
    pub fn input_reference_config(mut self, input: crate::types::IdNamespaceAssociationInputReferenceConfig) -> Self {
        self.input_reference_config = ::std::option::Option::Some(input);
        self
    }
    /// <p>The input reference configuration for the ID namespace association.</p>
    pub fn set_input_reference_config(mut self, input: ::std::option::Option<crate::types::IdNamespaceAssociationInputReferenceConfig>) -> Self {
        self.input_reference_config = input;
        self
    }
    /// <p>The input reference configuration for the ID namespace association.</p>
    pub fn get_input_reference_config(&self) -> &::std::option::Option<crate::types::IdNamespaceAssociationInputReferenceConfig> {
        &self.input_reference_config
    }
    /// <p>The input reference properties for the ID namespace association.</p>
    /// This field is required.
    pub fn input_reference_properties(mut self, input: crate::types::IdNamespaceAssociationInputReferenceProperties) -> Self {
        self.input_reference_properties = ::std::option::Option::Some(input);
        self
    }
    /// <p>The input reference properties for the ID namespace association.</p>
    pub fn set_input_reference_properties(
        mut self,
        input: ::std::option::Option<crate::types::IdNamespaceAssociationInputReferenceProperties>,
    ) -> Self {
        self.input_reference_properties = input;
        self
    }
    /// <p>The input reference properties for the ID namespace association.</p>
    pub fn get_input_reference_properties(&self) -> &::std::option::Option<crate::types::IdNamespaceAssociationInputReferenceProperties> {
        &self.input_reference_properties
    }
    /// <p>The configuration settings for the ID mapping table.</p>
    pub fn id_mapping_config(mut self, input: crate::types::IdMappingConfig) -> Self {
        self.id_mapping_config = ::std::option::Option::Some(input);
        self
    }
    /// <p>The configuration settings for the ID mapping table.</p>
    pub fn set_id_mapping_config(mut self, input: ::std::option::Option<crate::types::IdMappingConfig>) -> Self {
        self.id_mapping_config = input;
        self
    }
    /// <p>The configuration settings for the ID mapping table.</p>
    pub fn get_id_mapping_config(&self) -> &::std::option::Option<crate::types::IdMappingConfig> {
        &self.id_mapping_config
    }
    /// Consumes the builder and constructs a [`IdNamespaceAssociation`](crate::types::IdNamespaceAssociation).
    /// This method will fail if any of the following fields are not set:
    /// - [`id`](crate::types::builders::IdNamespaceAssociationBuilder::id)
    /// - [`arn`](crate::types::builders::IdNamespaceAssociationBuilder::arn)
    /// - [`membership_id`](crate::types::builders::IdNamespaceAssociationBuilder::membership_id)
    /// - [`membership_arn`](crate::types::builders::IdNamespaceAssociationBuilder::membership_arn)
    /// - [`collaboration_id`](crate::types::builders::IdNamespaceAssociationBuilder::collaboration_id)
    /// - [`collaboration_arn`](crate::types::builders::IdNamespaceAssociationBuilder::collaboration_arn)
    /// - [`name`](crate::types::builders::IdNamespaceAssociationBuilder::name)
    /// - [`create_time`](crate::types::builders::IdNamespaceAssociationBuilder::create_time)
    /// - [`update_time`](crate::types::builders::IdNamespaceAssociationBuilder::update_time)
    pub fn build(self) -> ::std::result::Result<crate::types::IdNamespaceAssociation, ::aws_smithy_types::error::operation::BuildError> {
        ::std::result::Result::Ok(crate::types::IdNamespaceAssociation {
            id: self.id.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "id",
                    "id was not specified but it is required when building IdNamespaceAssociation",
                )
            })?,
            arn: self.arn.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "arn",
                    "arn was not specified but it is required when building IdNamespaceAssociation",
                )
            })?,
            membership_id: self.membership_id.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "membership_id",
                    "membership_id was not specified but it is required when building IdNamespaceAssociation",
                )
            })?,
            membership_arn: self.membership_arn.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "membership_arn",
                    "membership_arn was not specified but it is required when building IdNamespaceAssociation",
                )
            })?,
            collaboration_id: self.collaboration_id.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "collaboration_id",
                    "collaboration_id was not specified but it is required when building IdNamespaceAssociation",
                )
            })?,
            collaboration_arn: self.collaboration_arn.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "collaboration_arn",
                    "collaboration_arn was not specified but it is required when building IdNamespaceAssociation",
                )
            })?,
            name: self.name.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "name",
                    "name was not specified but it is required when building IdNamespaceAssociation",
                )
            })?,
            description: self.description,
            create_time: self.create_time.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "create_time",
                    "create_time was not specified but it is required when building IdNamespaceAssociation",
                )
            })?,
            update_time: self.update_time.ok_or_else(|| {
                ::aws_smithy_types::error::operation::BuildError::missing_field(
                    "update_time",
                    "update_time was not specified but it is required when building IdNamespaceAssociation",
                )
            })?,
            input_reference_config: self.input_reference_config,
            input_reference_properties: self.input_reference_properties,
            id_mapping_config: self.id_mapping_config,
        })
    }
}
