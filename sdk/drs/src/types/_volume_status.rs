...
/// When writing a match expression against `VolumeStatus`, it is important to ensure
...
///
/// ```text
/// # let volumestatus = unimplemented!();
/// match volumestatus {
...
/// }
/// ```
/// The above code demonstrates that when `volumestatus` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `VolumeStatus::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `VolumeStatus::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `VolumeStatus::NewFeature` is defined.
/// Specifically, when `volumestatus` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `VolumeStatus::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::convert::From<&str> for VolumeStatus {
    fn from(s: &str) -> Self {
        match s {
...
impl ::std::str::FromStr for VolumeStatus {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(VolumeStatus::from(s))
    }
}
impl VolumeStatus {
...
impl ::std::convert::AsRef<str> for VolumeStatus {
    fn as_ref(&self) -> &str {
        self.as_str()
    }
}
